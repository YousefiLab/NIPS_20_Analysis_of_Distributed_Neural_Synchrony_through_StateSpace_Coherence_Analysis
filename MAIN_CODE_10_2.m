...
switch ch_sim
    case 5
        str_name_1 = sprintf('cfg_Init_chNum5_fr6_sr300_002_jh_c_winSec2_segNum10_TRIAL.mat',fr, sr, data_name, 2); % 5 channels
        str_name_2 = sprintf('Yk_chNum5_fr6_sr300_002_jh_c_winSec2_segNum1_TRIAL',fr, sr, data_name, 2);
        
    case 20
        str_name_1 = sprintf('cfg_Init_fr%d_sr%d_%s_winSec%d_segNum10_TRIAL.mat',fr, sr, data_name, 2); % 20 channels
        str_name_2 = sprintf('Yk_fr%d_sr%d_%s_winSec%d_segNum1_TRIAL',fr, sr, data_name, 2);
end
...
figure, hold on 
% plot(Lambda(1, :), marker{1}, 'LineWidth', 2)
% plot(Lambda(2, :), marker{2} , 'LineWidth', 2)
% plot(Lambda(3, :), marker{3}, 'LineWidth', 2)
% plot(Lambda(4, :), marker{4}, 'LineWidth', 2)
% plot(Lambda(5, :), marker{5}, 'LineWidth', 2)
...
lgd = legend('\lambda_1','\lambda_2','\lambda_3','\lambda_4','\lambda_5', 'Orientation','horizontal');
% lgd.FontSize = 60;

yyaxis left
for i=1: length(yy_vec)
    plot(i, 140, 'o', 'MarkerFaceColor', marker_col{yy_vec(i)}, 'MarkerEdgeColor', marker_col{yy_vec(i)}, 'MarkerSize', mrkr_size ); 
end
hold off
